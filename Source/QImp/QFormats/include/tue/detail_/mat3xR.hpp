...
namespace tue
{
	...
	public:
		...
		static constexpr int component_count = 3 * R;
		...
		static constexpr mat<T, 3, R> fill(const T& x) noexcept
		{
			return { vec<T, R>(x), vec<T, R>(x), vec<T, R>(x) };
		}

		static constexpr mat<T, 3, R> identity() noexcept
		{
			return {
		...
		static constexpr mat<T, 3, R> zero() noexcept
		{
			return {
		...
		template<typename J>
		void set_row(const J& j, const vec<T, 3>& v) noexcept
		{
		...
		mat<T, 3, R> operator++(int) noexcept
		{
			return {
					this->impl_.columns[0]++,
					this->impl_.columns[1]++,
		...
		template<typename U>
		std::enable_if_t<is_vec_component<U>::value, mat<T, 3, R>&>
		...
		template<typename U>
		mat<T, 3, R>&
		operator+=(const mat<U, 3, R>& m) noexcept
		{
	...
	namespace detail_
	{
		template<typename T, int R>
		inline constexpr
		mat<decltype(+std::declval<T>()), 3, R>
		unary_plus_operator_m(const mat<T, 3, R>& m) noexcept
		{
			return { +m[0], +m[1], +m[2] };
		}
		...
		inline constexpr
		mat<decltype(-std::declval<T>()), 3, R>
		unary_minus_operator_m(const mat<T, 3, R>& m) noexcept
		{
			return { -m[0], -m[1], -m[2] };
		}
		...
		inline constexpr
		mat<decltype(~std::declval<T>()), 3, R>
		bitwise_not_operator_m(const mat<T, 3, R>& m) noexcept
		{
			return { ~m[0], ~m[1], ~m[2] };
		}
		...
		inline constexpr
		mat<decltype(std::declval<T>() + std::declval<U>()), 3, R>
		addition_operator_xm(
				const T& lhs, const mat<U, 3, R>& rhs) noexcept
		{
			return {
					lhs + rhs[0],
					lhs + rhs[1],
		...
		addition_operator_mx(
				const mat<T, 3, R>& lhs, const U& rhs) noexcept
		{
			return {
					lhs[0] + rhs,
					lhs[1] + rhs,
		...
		addition_operator_mm(
				const mat<T, 3, R>& lhs, const mat<U, 3, R>& rhs) noexcept
		{
			return {
					lhs[0] + rhs[0],
					lhs[1] + rhs[1],
		...
		template<typename T, int R>
		inline mat<T, 3, R> sin_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::sin(m[0]),
					tue::math::sin(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> cos_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::cos(m[0]),
					tue::math::cos(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> exp_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::exp(m[0]),
					tue::math::exp(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> log_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::log(m[0]),
					tue::math::log(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> abs_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::abs(m[0]),
					tue::math::abs(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> recip_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::recip(m[0]),
					tue::math::recip(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> sqrt_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::sqrt(m[0]),
					tue::math::sqrt(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> rsqrt_m(const mat<T, 3, R>& m) noexcept
		{
			return {
					tue::math::rsqrt(m[0]),
					tue::math::rsqrt(m[1]),
		...
		template<typename T, int R>
		inline mat<T, 3, R> min_mm(
				const mat<T, 3, R>& m1, const mat<T, 3, R>& m2) noexcept
		{
			return {
		...
		template<typename T, typename U, int R>
		inline mat<U, 3, R> select_mmm(
				const mat<T, 3, R>& conditions,
				const mat<U, 3, R>& values,
				const mat<U, 3, R>& otherwise) noexcept
		{
			return {
		...
		inline mat<
				...
				3, R>
		less_mm(
				const mat<T, 3, R>& lhs, const mat<T, 3, R>& rhs) noexcept
		{
			return {
		...
		template<typename T>
		inline constexpr mat<T, 2, 3>
		transpose_m(const mat<T, 3, 2>& m) noexcept
		{
			return {
					{ m[0][0], m[1][0], m[2][0] },
					{ m[0][1], m[1][1], m[2][1] },
		...
		template<typename T>
		inline constexpr mat<T, 3, 3>
		transpose_m(const mat<T, 3, 3>& m) noexcept
		{
			return {
					{ m[0][0], m[1][0], m[2][0] },
					{ m[0][1], m[1][1], m[2][1] },
		...
		template<typename T>
		inline constexpr mat<T, 4, 3>
		transpose_m(const mat<T, 3, 4>& m) noexcept
		{
			return {
					{ m[0][0], m[1][0], m[2][0] },
					{ m[0][1], m[1][1], m[2][1] },
